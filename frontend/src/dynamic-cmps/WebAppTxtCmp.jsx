...
export const WebAppTxtCmp = ({ cmp, currCmp, onDeleteCmp, onSetCurrCmp }) => {
    ...
    if (currCmp && currCmp.id === cmp.id) {
        return (
            <span id={cmp.id} style={{ ...cmp.attributes.style, position: 'relative', outline: '2px dashed #c6c6c6' }} className={cmp.attributes.className} onClick={(ev) => { onSetCurrCmp(ev, cmp) }} >
                <ElementToolBar />
                {cmp.info.txt}
            </span>
        )
