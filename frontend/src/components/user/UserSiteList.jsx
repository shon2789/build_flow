    ...
    return (
        <>
            ...
            <main className="user-site-list">
                ...
                {(user?.webApps.length > 0 && filterBy === 'saved') && user.webApps.filter(webApp => !webApp?.isPublished).map( webApp => {
                    return <UserSitePreview key={webApp._id} webApp={webApp} />
                })
                }
                {(user?.webApps.length > 0 && filterBy === 'published') && user.webApps.filter(webApp => webApp?.isPublished).map( webApp =>{
                    return <UserSitePreview key={webApp._id} webApp={webApp} />
                })
                }
            </main>
